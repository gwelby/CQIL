...
function drawBlochSphere() {
    ...
    superpositionCtx.arc(centerX, centerY, radius, 0, Math.PI * 2);
    ...
    // X axis (red)
    superpositionCtx.strokeStyle = 'rgba(244, 67, 54, 0.7)';
    superpositionCtx.beginPath();
    superpositionCtx.moveTo(centerX - radius, centerY);
    superpositionCtx.lineTo(centerX + radius, centerY);
    superpositionCtx.stroke();
    
    // Z axis (blue)
    superpositionCtx.strokeStyle = 'rgba(33, 150, 243, 0.7)';
    superpositionCtx.beginPath();
    superpositionCtx.moveTo(centerX, centerY - radius);
    superpositionCtx.lineTo(centerX, centerY + radius);
    superpositionCtx.stroke();
    
    // Draw poles (|0⟩ and |1⟩)
    superpositionCtx.fillStyle = 'rgba(33, 150, 243, 0.9)';
    superpositionCtx.beginPath();
    superpositionCtx.arc(centerX, centerY - radius, 8, 0, Math.PI * 2);
    superpositionCtx.fill();
    
    superpositionCtx.fillStyle = 'rgba(244, 67, 54, 0.9)';
    superpositionCtx.beginPath();
    superpositionCtx.arc(centerX, centerY + radius, 8, 0, Math.PI * 2);
    superpositionCtx.fill();
    ...
    superpositionCtx.fillText('|0⟩', centerX, centerY - radius - 15);
    superpositionCtx.fillText('|1⟩', centerX, centerY + radius + 25);
    
    // Only draw Bloch vector if we're dealing with a 2-state system
    if (superpositionStates === 2 && superpositionParticles.length > 0) {
        // Draw Bloch vector
        const vectorX = centerX + blochSphere.x * radius;
        const vectorY = centerY - blochSphere.z * radius; // Flip z since canvas y is inverted
        
        superpositionCtx.strokeStyle = 'rgba(255, 193, 7, 0.9)';
        superpositionCtx.lineWidth = 3;
        superpositionCtx.beginPath();
        superpositionCtx.moveTo(centerX, centerY);
        superpositionCtx.lineTo(vectorX, vectorY);
        superpositionCtx.stroke();
        
        // Draw arrowhead
        const angle = Math.atan2(vectorY - centerY, vectorX - centerX);
        superpositionCtx.fillStyle = 'rgba(255, 193, 7, 0.9)';
        superpositionCtx.beginPath();
        superpositionCtx.moveTo(vectorX, vectorY);
        superpositionCtx.lineTo(
            vectorX - 15 * Math.cos(angle - Math.PI / 6),
            vectorY - 15 * Math.sin(angle - Math.PI / 6)
        );
        superpositionCtx.lineTo(
            vectorX - 15 * Math.cos(angle + Math.PI / 6),
            vectorY - 15 * Math.sin(angle + Math.PI / 6)
        );
        superpositionCtx.closePath();
        superpositionCtx.fill();
        
        // Draw point at end of vector
        superpositionCtx.fillStyle = 'rgba(255, 193, 7, 0.9)';
        superpositionCtx.beginPath();
        superpositionCtx.arc(vectorX, vectorY, 8, 0, Math.PI * 2);
        superpositionCtx.fill();
    }
    
    // Draw equator (for reference)
    superpositionCtx.strokeStyle = 'rgba(255, 255, 255, 0.2)';
    superpositionCtx.lineWidth = 1;
    superpositionCtx.beginPath();
    superpositionCtx.ellipse(centerX, centerY, radius, radius * 0.3, 0, 0, Math.PI * 2);
    superpositionCtx.stroke();
    
    // If multiple states (>2), draw them arranged in a circle
    if (superpositionStates > 2 && superpositionParticles.length > 0) {
        const states = superpositionParticles[0].states;
        const time = performance.now() / 1000;
        
        // Draw connecting lines from center to each state
        superpositionCtx.lineWidth = 1;
        ...
        for (let i = 0; i < states.length; i++) {
            ...
            // Draw connection line with phase-dependent coloring
            const phaseOffset = state.phase;
            const phaseColor = `hsl(${(phaseOffset / (Math.PI * 2)) * 360}, 80%, 60%)`;
            
            superpositionCtx.strokeStyle = state.color;
            superpositionCtx.globalAlpha = state.amplitude;
            superpositionCtx.beginPath();
            superpositionCtx.moveTo(centerX, centerY);
            superpositionCtx.lineTo(stateX, stateY);
            superpositionCtx.stroke();
            superpositionCtx.globalAlpha = 1.0;
            
            // Draw state point with size based on probability
            const stateSize = 5 + state.amplitude * 15;
            superpositionCtx.fillStyle = state.color;
            superpositionCtx.beginPath();
            superpositionCtx.arc(stateX, stateY, stateSize, 0, Math.PI * 2);
            superpositionCtx.fill();
            
            // Draw phase indicator (rotating segment)
            superpositionCtx.strokeStyle = 'white';
            superpositionCtx.lineWidth = 2;
            superpositionCtx.beginPath();
            superpositionCtx.moveTo(stateX, stateY);
            superpositionCtx.lineTo(
                stateX + Math.cos(time * 2 + phaseOffset) * stateSize * 1.5,
                stateY + Math.sin(time * 2 + phaseOffset) * stateSize * 1.5
            );
            superpositionCtx.stroke();
            ...
            superpositionCtx.fillText(
                state.label,
                stateX + Math.cos(angle) * 25,
                stateY + Math.sin(angle) * 25
            );
            
            // Draw probability percentage near state
            superpositionCtx.font = '12px Arial';
            superpositionCtx.fillText(
                `${Math.round(state.probability * 100)}%`,
                stateX + Math.cos(angle) * 40,
                stateY + Math.sin(angle) * 40
            );
        }
        
        // Draw central particle
        superpositionCtx.fillStyle = 'rgba(255, 255, 255, 0.9)';
        superpositionCtx.beginPath();
        superpositionCtx.arc(centerX, centerY, 15, 0, Math.PI * 2);
        superpositionCtx.fill();
        
        // Draw pulsing coherence field
        const coherenceFactor = superpositionCoherence / 100;
        const pulseSize = (0.6 + Math.sin(time * 3) * 0.1) * coherenceFactor;
        
        superpositionCtx.fillStyle = `rgba(255, 255, 255, ${0.1 * coherenceFactor})`;
        superpositionCtx.beginPath();
        superpositionCtx.arc(centerX, centerY, radius * pulseSize, 0, Math.PI * 2);
        superpositionCtx.fill();
    }
}
